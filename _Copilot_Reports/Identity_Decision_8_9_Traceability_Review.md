...
## 10. Administrative recovery proposal (ADR-0004 v1.1.0)

ADR-0004 now requires an internal `AdminRecoverStalledRegistration` design before acceptance, rather than offering acceptance of indefinite waiting alone. The two actions invalidate an uncommitted verification attempt under its existing consumption guard, or reconcile a committed registration/Ready acknowledgment through the canonical C01–C03 path. Mandatory operator authorization, durable request identity, append-only audit and bounded support outcomes are specified in Decision 4. This is a new proposed operational entry point, not implicit authority derived from the previous Blueprint.

Post-commit cancellation/revocation is deliberately not granted: it would need to arbitrate with an in-flight Identity Ready transaction as well as the Credential guard. The design cannot guarantee automatic repair of permanent integrity damage. Blueprint/API/machine/security/runbook propagation and verification are still pending; no ADR is accepted by adding this design. PR #5 remains the sole Draft review vehicle; #1–#4 are superseded, not merged.

## Owner approval and documentary propagation — 2026-09-25

The owner accepted ADR-0004 v1.1.0 at pinned source 82ef455 through the [acceptance record](Identity_ADR-0004_Acceptance_Decision_Record.md), introduced at commit 16b720c9cb9afdd60769dcad7b2c4d8c1e2e983c. Commit cca3f7ad0241111e468ef2b5a5d76e67de8d7cec records Accepted metadata v1.1.1. A01/A02/A03 and Decision 4 are architecturally accepted within that record's scope. The earlier findings remain evidence of their pinned review states, not current assertions of missing architectural approval. No independent external review is claimed.

The subsequent Blueprint/API/service-schema/runbook propagation is documented in [the propagation report](Identity_ADR-0004_Propagation_Validation_2026-09-25.md). Concrete encodings remain Draft. T16, ADR-0002 acceptance, operational values, consumer compatibility, full validation and runtime/security tests remain open. Administrative recovery does not authorize post-commit cancellation or direct database repair. PR #5 is the sole Draft review path; #1–#4 were closed without merging.
